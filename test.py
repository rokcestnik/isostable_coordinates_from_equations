...
def dx(state):
	...
	#return state[1] # vdp
	#return 1.5+state[0]**2*state[1]-3.5*state[0]-state[0] # brusselator (slow)
...
def dy(state):
	...
	#return 3.5*state[0]-state[0]**2*state[1] # brusselator (slow)
...
#isochrons, isostables = oscillator_isochrons_isostables(ders, isochron_resolution=10, number_of_isostables=5, amplitude_unit=0.06, amplitude_domain=5*0.06, dt=0.003) # brusellator (slow)
