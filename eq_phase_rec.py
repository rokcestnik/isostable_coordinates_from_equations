...
def oscillator_PRC(ders, direction, initial_state=None, initial_warmup_periods=10, stimulation=0.05, warmup_periods=3, dph=0.1, thr=0.0, dt=0.005):
	...
	for s in range(len(PRC[0])):
		...
		PRC[1][s] = (oscillator_phase(state_stim, ders, period)-PRC[0][s])%(2*pi)/stimulation
	return PRC



def oscillator_PRC2(ders, direction, initial_state=None, warmup_time=1000.0, stimulation=1.0, period_counts=5, dph=0.1, thr=0.0, dt=0.01):
	"""calculates the phase response curve from dynamical equations
	...
	for ph in [dph*i for i in range(floor(2*pi/dph))]:
		...
		state = one_step_integrator(state_stim, ders, dt)
		time = time + dt
...
def oscillator_ARC(ders, direction, initial_state=None, warmup_time=1000.0, stimulation=1.0, period_counts=5, dph=0.1, thr=0.0, dt=0.01):
